...
예시 3)
대화:
[user] DNA 복제는 어떻게 일어나?
[assistant] 헬리케이스, 폴리머라제 등 여러 효소가 관여합니다.
[user] 그중 폴리머라제가 하는 일을 알려줘.
검색 쿼리: DNA 복제 폴리머라제 역할 합성
"""
...
- "여행을 통해 얻을 수 있는 좋은 점들이 뭐가 있어?" → yes (문화)
- "삼각형 내각의 합은 몇 도야?" → yes (수학)
- "블록체인이 뭔데?" → yes (IT·기술)
...
def is_science_question(msg: list[dict], llm: _LLMComplete) -> bool:
    ...
    if len(user_msgs) > 1:
        ...
        prompt = (
            ...
            "마지막 발화가 짧거나 '그것/그거/이거/위의'처럼 불완전해도, "
            "앞 대화 맥락에서 사실·원리·정의를 묻는 것이면 yes입니다. "
...
다음 대화에서 마지막 사용자 질문만을 반영해, 검색 엔진에 넣을 **독립적인 한 줄 쿼리**만 출력하세요.
- '그것/그거/이거/위의/방금 말한 것' 등은 이전 대화에서 **대상을 찾아** 구체적인 명사·용어로 바꾸세요.
- 질문에 나온 **고유명사·수치·단위·화학식·기호**는 가능하면 그대로 유지하세요.
- 질문에 없는 주제·인물·사실을 **새로 만들어 넣지 마세요**.
- 핵심 과학·학문 키워드 위주로 간결하게 쓰고, 인사·감정 표현·불필요한 수식어는 빼세요.
...
def generate_alt_query(query: str, llm: _LLMComplete) -> str:
    ...
    """
    fusion_prompt = (
        "다음 과학·학문 질문을 **다른 표현으로 한 줄만** 재작성하세요.\n"
        "- 질문의 **핵심 주제와 대상(엔티티)** 은 바꾸지 마세요.\n"
        "- 질문에 있는 **고유명사·수치·단위**는 그대로 두세요.\n"
        "- 질문에 없는 새로운 개념이나 이름을 추가하지 마세요.\n"
        f"질문: {query}\n재작성:"
    )
