...
flowchart LR
    ...
    subgraph pipeline ["② ~ ⑤ 파이프라인 (4-Phase 순차 로드)"]
        ...
        RET["Phase 1: Hybrid Retrieval<br>Embedding 8B + BM25<br>→ RRF(k=60) Top-100"]
        RNK["Phase 2: Reranking<br>Qwen3-Reranker-8B<br>+ Soft Voting → Top-20"]
...
├── .env.example                ← 시크릿 템플릿 (ES_PASSWORD, OPENAI_API_KEY)
├── .gitignore
...
cp .env.example .env
# ES_PASSWORD, OPENAI_API_KEY, HF_TOKEN 입력
# OpenAI 쿼터 초과 시: DISABLE_SELFCHECK=1 추가
...
**Q. RAGAS Self-check가 OpenAI 오류로 실패합니다.**  
A. RAGAS Faithfulness는 OpenAI API를 평가자로 사용합니다. OpenAI 쿼터 초과 시 `.env`에 `DISABLE_SELFCHECK=1`을 추가하면 Self-check 없이 첫 번째 생성 결과를 반환합니다.
